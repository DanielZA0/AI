...
epochs_kmeans = 600 # epocas del aprendizaje no supervisado
...
cluster3 = matrix[np.where(min_d==2)]
# Visualizacion de datos
plt.figure(5)
ax = plt.axes(projection ="3d")
ax.scatter3D(cluster1[:,1], cluster1[:,2], cluster1[:,4], c='red', label='Clase a')
ax.scatter3D(cluster2[:,1], cluster2[:,2], cluster2[:,4], c='blue', label='Clase b')
ax.scatter3D(cluster3[:,1], cluster3[:,2], cluster3[:,4], c='green', label='Clase c')
